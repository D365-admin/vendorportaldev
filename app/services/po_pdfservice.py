...
def fetch_po_pdf(purch_id: str):
    ...
    if not row:
        raise HTTPException(
            status_code=404,
            detail=f"No PO document found for {purch_id}"
        )

    file_bytes = row.FileContent

    # Handle hex-encoded binary (pyodbc sometimes returns this)
    if isinstance(file_bytes, str):
        file_bytes = bytes.fromhex(
            file_bytes.replace("0x", "").replace("0X", "")
        )
    elif isinstance(file_bytes, memoryview):
        file_bytes = bytes(file_bytes)

    file_name    = row.FileName    or f"{purch_id}.pdf"
