...
def fetch_submitted_rfq_detail(
    ...
    try:
        with get_connection() as conn:
            ...
            # --------------------------------------------------------
            # FIX 1: Header query — use subquery on d365_PurchRFQCaseTable
            # to avoid row multiplication when RFQCASEID has multiple rows.
            # Only TOP 1 by CREATEDDATETIME ASC is taken for ISSUE_DATE.
            # --------------------------------------------------------
            cur.execute(f"""
                SELECT TOP 1
                    H.ID AS HEADERID,
                ...
                LEFT JOIN (
                    SELECT TOP 1
                        RFQCASEID,
                        CREATEDDATETIME
                    FROM {SCHEMA}.d365_PurchRFQCaseTable WITH (NOLOCK)
                    ORDER BY CREATEDDATETIME ASC
                ) L ON L.RFQCASEID = H.RFQCASEID
            ...
            # --------------------------------------------------------
            # FIX 2: Line query — use ROW_NUMBER() on D365_PURCHRFQLINE
            # partitioned by RFQID + LINENUM + ITEMID, pick RN=1 only.
            # This prevents each bid line from multiplying by the number
            # of D365_PURCHRFQLINE revisions for that item.
            # --------------------------------------------------------
            cur.execute(f"""
                SELECT
                    L.LINENUMBER,
